...
def selects_oldest_bear_and_returns_name_and_age
  "SELECT b1.name, b1.age FROM bears b1 WHERE b1.age = (SELECT MAX(age) FROM bears) LIMIT 1"
end

def select_youngest_bear_and_returns_name_and_age
  "SELECT b1.name, b1.age FROM bears b1 WHERE b1.age = (SELECT MIN(age) FROM bears)"
end
